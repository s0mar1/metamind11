...
const styles = {
  ...
  title: { fontSize: '2rem', fontWeight: 'bold', textAlign: 'center', marginBottom: '0.5rem', color: '#2E2E2E' },
  ...
  unitWrapper: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', width: '56px' },
  ...
  col4: { flexShrink: 0 },
  expandButton: { color: '#6E6E6E', fontSize: '1.5rem', background: 'none', border: 'none', cursor: 'pointer', padding: '8px', transition: 'transform 0.2s' },
};
...
function TierListPage() {
  ...
  useEffect(() => {
    const fetchTierData = async () => {
      ...
      } catch (err) { setError('덱 티어 정보를 불러오는 데 실패했습니다.'); }
  ...
  if (error) return <div style={{...styles.loadingError, color: styles.error.color}}>{error}</div>;

  return (
    <div style={styles.container}>
      <h1 style={styles.title}>실시간 덱 티어리스트</h1>
      <p style={styles.subtitle}>최신 1시간 데이터 기반 상위 랭커들의 메타 분석 정보입니다.</p>
      {tierData.length > 0 ? (
